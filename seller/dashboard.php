...
echo '<link rel="stylesheet" href="/mobileshop/inc/style.css">';
...
const hiddenSlugs = ['weight', 'weight_notebook', 'nb_ssd_size', 'nb_hdd_size', 'nb_gpu'];
    ...
    try {
        ...
        if (data.values && data.values.length) {
            ...
            data.values.forEach(v => {
                selectElement.innerHTML += `<option value="${escapeHtml(v.value)}">${escapeHtml(v.value)}</option>`;
            });
        } else {
    ...
    if (val === 'SSD') {
        container.insertAdjacentHTML('beforeend', `
            <div class="col-md-6 mb-3" data-dependent="ssd">
                ...
                <div class="input-group">
                    <input type="number" name="attributes[${attrId}][ssd]" step="1" class="form-control" required min="0">
        ...
        container.insertAdjacentHTML('beforeend', `
            <div class="col-md-6 mb-3" data-dependent="hdd">
                ...
                <div class="input-group">
                    <input type="number" name="attributes[${attrId}][hdd]" step="1" class="form-control" required min="0">
                    <span class="input-group-text">ГБ</span>
                </div>
            </div>
        `);
    ...
    if (val === 'Встроенная') {
        container.insertAdjacentHTML('beforeend', `
            <div class="col-md-6 mb-3" data-dependent="gpu-integrated">
                ...
                <input type="text" name="attributes[${attrId}][integrated]" class="form-control" placeholder="Intel UHD Graphics" required>
            </div>
        `);
        ...
        container.insertAdjacentHTML('beforeend', `
            <div class="col-md-6 mb-3" data-dependent="gpu-discrete">
                ...
                <input type="text" name="attributes[${attrId}][discrete]" class="form-control" placeholder="NVIDIA GeForce RTX 3060" required>
            </div>
        `);
        ...
        container.insertAdjacentHTML('beforeend', `
            <div class="col-md-6 mb-3" data-dependent="gpu-integrated">
                <label class="form-label">Встроенная видеокарта <span class="text-danger">*</span></label>
                <input type="text" name="attributes[${attrId}][integrated]" class="form-control" placeholder="Intel Iris Xe" required>
            </div>
            <div class="col-md-6 mb-3" data-dependent="gpu-discrete">
                <label class="form-label">Дискретная видеокарта <span class="text-danger">*</span></label>
                <input type="text" name="attributes[${attrId}][discrete]" class="form-control" placeholder="NVIDIA GeForce RTX 3060" required>
...
function getUnitDisplay(attr) {
    ...
    if (attr.unit && attr.unit !== '') return attr.unit;
    return '';
}
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function renderCategoryAttributes() {
        ...
        const cid = parseInt(categorySelect.value, 10);

        if (!cid) {
            attributesContainer.innerHTML = '<div class="alert alert-info">Выберите категорию, чтобы отобразить характеристики.</div>';
            return;
        }

        const rawAttrs = categoryAttributes[cid] || [];
        const attrs = rawAttrs.filter(a => !hiddenSlugs.includes(a.slug));

        if (attrs.length === 0) {
            attributesContainer.innerHTML = '<div class="alert alert-secondary mb-0">Для выбранной категории дополнительные характеристики не предусмотрены.</div>';
            return;
        }

        attrs.sort((a, b) => Number(a.sort_order) - Number(b.sort_order));
        ...
        setTimeout(() => {
            document.querySelectorAll('select[data-attr-id]').forEach(s => {
                loadSelectValues(s.dataset.attrId, s);
            });
