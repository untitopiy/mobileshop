...
<div class="reviews-list">
    ...
    <?php if($current_user_id): ?>
        <button class="btn btn-sm btn-outline-success me-2 btn-like-review <?= !empty($review['is_liked_by_user']) ? 'active' : '' ?>" 
                data-review-id="<?= $review['id'] ?>"
                data-action="helpful">
            <i class="fas fa-thumbs-up me-1"></i>
            Да (<span class="likes-count"><?= $review['likes_count'] ?? 0 ?></span>)
        </button>
        
        <button class="btn btn-sm btn-outline-danger btn-dislike-review <?= !empty($review['is_disliked_by_user']) ? 'active' : '' ?>" 
                data-review-id="<?= $review['id'] ?>"
                data-action="not-helpful">
            <i class="fas fa-thumbs-down me-1"></i>
            Нет (<span class="dislikes-count"><?= $review['dislikes_count'] ?? 0 ?></span>)
        </button>
    <?php else: ?>
        <div class="text-muted small">
            <i class="fas fa-thumbs-up me-1"></i> <?= $review['likes_count'] ?? 0 ?>
            <i class="fas fa-thumbs-down ms-2 me-1"></i> <?= $review['dislikes_count'] ?? 0 ?>
            <span class="ms-2">(Авторизуйтесь, чтобы оценить)</span>
        </div>
    <?php endif; ?>
</div>
...
/* Делаем инпуты невидимыми, но оставляем их в DOM для браузера */
.rating-input input {
    position: absolute;
    opacity: 0;
    width: 0;
