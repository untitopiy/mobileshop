    ...
    if (empty($errors)) {
        ...
        if ($stmt->num_rows > 0) {
            ...
            if (password_verify($password, $hashed_password)) {
                ...
                // Определяем, является ли пользователь активным продавцом
                $redirect_url = '../index.php';

                $seller_check = $db->prepare("SELECT id FROM sellers WHERE user_id = ? AND is_active = 1");
                if ($seller_check) {
                    $seller_check->bind_param('i', $id);
                    $seller_check->execute();
                    $seller_check->store_result();
