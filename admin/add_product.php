...
// Получаем данные и сохраняем в массивы
$categories_data = [];
...
$sellers_data = [];
$sellers_result = $db->query("SELECT id, shop_name FROM sellers WHERE is_active = 1 ORDER BY shop_name");
...
$selected_seller = isset($_POST['seller_id']) ? $_POST['seller_id'] : '';
...
<div class="container-fluid admin-container my-5">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <a href="manage_products.php" class="btn btn-secondary"><i class="fas fa-arrow-left"></i> Назад</a>
    </div>
            ...
            <div class="col-md-4">
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        ...
                        <label class="form-label mt-3">Продавец</label>
                        <select name="seller_id" class="form-select">
                            <option value="">— Администратор —</option>
                            <?php foreach ($sellers_data as $seller): ?>
                                <option value="<?= $seller['id'] ?>" <?= $selected_seller == $seller['id'] ? 'selected' : '' ?>>
...
function getStepForAttribute(attr) {
    ...
    // Шаг 0.5 для времени работы наушников
    if (attr.slug === 'hp_battery_life') {
        return '0.5';
    }
    // Шаг 0.1 для остальных числовых полей
    return '0.1';
}

function getUnitDisplay(attr) {
    // Принудительное отображение для быстрой зарядки
    if (attr.slug === 'sm_fast_charging') {
        return 'Вт';
    }
    // Для остальных атрибутов используем unit из БД
    ...
    if (categorySelect) {
        categorySelect.addEventListener('change', function() {
            ...
            setTimeout(() => {
                ...
                // Повторная инициализация tooltips для новых элементов
